...
export default function BoardPage({ location, history }) {
    ...
    const [num, setNum] = useState(parseInt(query.notice_id))
    console.log(num)
    ...
    useEffect(() => {
        ...
        if (parseInt(query.notice_id) !== num) {
            setNum(parseInt(query.notice_id))
            getAxiosData(`api/notice/notice_view?notice_id=${query.notice_id}`)
        }
    ...
    }, [query.no, num, article, history, page])
    return (
        <div>
