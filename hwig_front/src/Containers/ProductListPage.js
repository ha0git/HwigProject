...
function ProductListPage(props) {
    ...
    useEffect(() => {
        ...
        if ((parseInt(query.category_id) !== list) || (parseInt(query.page) !== page)) {
            ...
            if(query.category_id === list ){
                getAxiosData(`api/product/mainlist?cate_id=${list}`)
            }
            else if (query.category_id === list || query.productnew === null) {
    ...
    return (
        <>
            {productItems && <ProductList productItems={productItems} page={page} size={size} history={props.history} />}
        </>
    )
}
